...
async function run() {
    ...
    try {
        const result = parseToolResult(await client.callTool('browse', {action: 'open', url: TEST_URL, wait: 'load'}))
        if (result?.success) {
    ...
    if (testTabId) {
        ...
        // 1.4 attach（前台）
        try {
            const result = parseToolResult(await client.callTool('browse', {action: 'attach', targetId: testTabId, activate: true}))
    ...
    if (testTabId && testTabId2) {
        try {
            ...
            if (content1.includes('Text Box') && content2.includes('Button')) {
                ...
                reporter.fail('multi-tab: extract text from different tabs',
                    `Tab1 has "Text Box": ${content1.includes('Text Box')}, Tab2 has "Button": ${content2.includes('Button')}`)
            }
        } catch (e) {
