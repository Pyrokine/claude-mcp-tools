...
export class ExtensionHttpServer extends EventEmitter {
    ...
    private handleMessage(data: string): void {
        try {
            const message = JSON.parse(data) as {id?: string; success?: boolean; data?: unknown; error?: string; type?: string; version?: string}
    ...
    async sendCommand(action: string, params: unknown = {}, requestTimeout?: number): Promise<unknown> {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                this.messageHandlers.delete(messageId)
                reject(new Error(`Failed to send command ${action}: ${err instanceof Error ? err.message : 'Unknown error'}`))
            }
        })
    }
